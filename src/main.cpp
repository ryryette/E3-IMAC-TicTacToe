...
int ia_choix_case(char const cases[9], char joueurSym);
int ia_blocage_case_restante(char const cases[9], char joueurSym);
    ...
    */
    ...
    while (true)
    {
        ...
        */
        ...
        {
            int choix_ia = ia_choix_case(cases, joueur1.symbole);
...
int ia_blocage_case_restante(char const cases[9], char joueurSym)
{
    auto check = [&](int a, int b, int c) -> int {
        ...
        if (cases[a] == joueurSym) countSym++;
        else if (cases[a] >= '1' && cases[a] <= '9') { countEmpty++; emptyIndex = a; }

        if (cases[b] == joueurSym) countSym++;
        else if (cases[b] >= '1' && cases[b] <= '9') { countEmpty++; emptyIndex = b; }

        if (cases[c] == joueurSym) countSym++;
        else if (cases[c] >= '1' && cases[c] <= '9') { countEmpty++; emptyIndex = c; }
    ...
    int r;
    if ((r = check(0,1,2))) return r;
    if ((r = check(3,4,5))) return r;
    if ((r = check(6,7,8))) return r;

    if ((r = check(0,3,6))) return r;
    if ((r = check(1,4,7))) return r;
    if ((r = check(2,5,8))) return r;

    if ((r = check(0,4,8))) return r;
    if ((r = check(2,4,6))) return r;
...
int ia_choix_case(char const cases[9], char joueurSym)
{
    int bloc = ia_blocage_case_restante(cases, joueurSym);
    if (bloc != 0)
        return bloc;
